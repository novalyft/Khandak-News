import { NextRequest, NextResponse } from 'next/server';

// Revalidate every 2 minutes
export const revalidate = 120;

const SERVER_BASE_URL = 'http://46.62.165.97:1337';
const TOKEN = `Bearer `;
const INFOGRAPHS_URL = 'api/infographs';

export async function GET(request) {
  try {
    // Get query parameters
    const { searchParams } = new URL(request.url);
    const page = searchParams.get('page') || '1';
    const pageSize = searchParams.get('pageSize') || searchParams.get('limit') || '25';
    const locale = searchParams.get('locale');

    // Build query parameters for Strapi API
    const params = new URLSearchParams({
      'pagination[page]': page,
      'pagination[pageSize]': pageSize,
      'sort[0]': 'createdAt:desc', // Sort from recent to old
    });

    // Add locale if provided
    if (locale) {
      params.append('locale', locale);
    }

    // Construct the full URL to the server with populate for coverImage
    const serverUrl = `${SERVER_BASE_URL}/${INFOGRAPHS_URL}?populate[0]=coverImage&${params.toString()}`;

    // Fetch infographs from the server
    const response = await fetch(serverUrl, {
      method: 'GET',
      headers: {
        'Authorization': TOKEN,
        'Content-Type': 'application/json',
        'User-Agent': 'Kandak-News-Infographs-Proxy/1.0',
      },
      next: { revalidate: 120 }, // Revalidate every 2 minutes
    });

    if (!response.ok) {
      const errorText = await response.text();
      console.error('Infographs API error:', response.status, errorText);
      return NextResponse.json(
        { error: 'Failed to fetch infographs', status: response.status },
        { status: response.status }
      );
    }

    // Get the JSON data
    const data = await response.json();

    // Return the data with appropriate headers
    return NextResponse.json(data, {
      status: 200,
      headers: {
        'Content-Type': 'application/json',
        'Cache-Control': 'public, s-maxage=60, stale-while-revalidate=300',
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': 'GET, OPTIONS',
        'Access-Control-Allow-Headers': 'Content-Type',
      },
    });
  } catch (error) {
    console.error('Infographs proxy error:', error);
    return NextResponse.json(
      { error: 'Internal Server Error', message: error.message },
      { status: 500 }
    );
  }
}

// Handle OPTIONS requests for CORS
export async function OPTIONS(request) {
  return new NextResponse(null, {
    status: 200,
    headers: {
      'Access-Control-Allow-Origin': '*',
      'Access-Control-Allow-Methods': 'GET, OPTIONS',
      'Access-Control-Allow-Headers': 'Content-Type',
    },
  });
}

